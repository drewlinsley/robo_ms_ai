...
class HeatMap:
    def __init__(self,image,heat_map,gaussian_std=10, vmin=0, vmax=1, mask=0.01):
    ...
    def plot(self,transparency=0.7,color_map='bwr',
        ...
        self.heat_map/=255.
        if self.mask:
            self.heat_map = np.ma.masked_where(self.heat_map <= self.mask, self.heat_map)
        plt.imshow(self.heat_map, alpha=transparency, cmap=color_map, vmax=self.vmax, vmin=self.vmin)
        if show_colorbar:
            plt.colorbar()
